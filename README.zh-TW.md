...
每個 issue 都會依證據品質打 0–100 分（行號、引用規則、可重現性），只有 ≥ 80 分的 issue 會顯示——過濾雜訊、保留訊號。若專案有 `CLAUDE.md` 或 `Agents.md`，會自動檢查規範合規性。
...
**信心度評分**（v0.9.0）：所有 provider 的每個 issue 都會被打 0–100 分，基於**證據品質**而非 Claude 的觀點。只有 ≥ 80 分的 issue 會進入最終輸出。評分依據：具體行號（+25）、本次 diff 引入的程式碼（+25）、引用規則（+20）、可重現場景（+15）、多方共識（+20）。過濾雜訊的同時保留 cross-provider 盲點消除的核心價值。
