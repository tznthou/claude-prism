...
| `/ui-design` | Gemini | 從需求生成 UI/UX 設計規格 |
...
### `/ui-design` — UI/UX 設計規格生成

Gemini 從需求描述生成結構化、可直接實作的 UI/UX 設計規格。輸出涵蓋資訊架構、ASCII wireframe、元件拆解、互動設計、視覺方向和實作建議。可選 `--html` 產出 HTML 原型。

```
/ui-design 一個 SaaS dashboard，含數據分析圖表和使用者管理
/ui-design docs/requirements.md
/ui-design --html 開發者工具的 landing page
```
