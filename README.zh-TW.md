...
```
你 ↔ Claude Code ─── /ask-codex ──────→ Codex CLI (GPT-5.3)
                 ├── /ask-gemini ─────→ Gemini CLI (3 Pro)
                 ├── /code-review ────→ Codex review Claude 寫的程式碼
                 ├── /ui-review ──────→ Gemini 審查 UI/UX
                 ├── /research ───────→ Gemini 研究替代方案
                 └── /multi-review ───→ 雙方 + Claude 整合分析
```
