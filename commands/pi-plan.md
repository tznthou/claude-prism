...
- If one provider fails → continue with the other + Claude. Include the specific failure reason from stderr (TIMEOUT, RATE_LIMIT, AUTH_ERROR, etc.). Note: "⚠️ [Provider] unavailable ([reason]) — plan informed by [other provider] + Claude."
