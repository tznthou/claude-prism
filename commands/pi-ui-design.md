...
If Gemini fails (script exits non-zero or CLI not found), Claude generates the HTML mockup directly using the same requirements (Tailwind CDN, responsive, semantic HTML5, realistic content). Note in output: "Gemini unavailable — mockup generated by Claude only."
